...
public class Form extends CommonElement implements FlowContent, PhrasingContent {
	...
	public Form(GlobalAttribute[] ga, AcceptCharset ac, Action action,
			AutoComplete autocomplete, EncType enctype, Method method,
			Name name, NoValidate novalidate, Target target) {
		super(ga);
		this.ac = ac;
		this.action = action;
	...
	protected Stream<Attribute> getElementSpecificAttributes() {
		return Stream.of(ac, action, autocomplete, enctype, method, name,
				novalidate, target).filter(Objects::nonNull);
	}
}
